	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
		if (isset($_POST['username']) &&
			...
			if (try_login($username, $password)) {
				...
				$model['response'] = "Nie znaleziono takiego użytkownika!";
			}
		}
	}    
	return 'login_view';
...
function register(&$model) {
   ...
   if (isset($_POST['username']) &&
		...
		if ($password <> $_POST['passwordAgain']) {
			$model['response'] =  "Hasła muszą być takie same! </br>";
			return 'register_view';
		}
		if (user_exists($_POST['username'], "", true)) {
			$model['response'] = "Użytkownik już istnieje!";
			return 'register_view';
		}
		...
		if (register_user($user)) {
			$model['response'] = "Pomyślnie zarejestrowano!";
		}
		else {
			$model['response'] = "Użytkownik o takiej nazwie już istnieje!";
		}
	}
	return 'register_view';
}
